<x-layouts.guest>
    ...
    @slot('right')
        <x-layouts.form method="POST" action="{{ route('register-user') }}">
            ...
            <x-forms.fieldset :title="__('Password')" icon="key">
                <x-forms.input name="password" type="password" :placeholder="__('form.password')" />
                <x-forms.input name="password_confirmation" type="password" :placeholder="__('form.confirm_password')" />
            </x-forms.fieldset>
